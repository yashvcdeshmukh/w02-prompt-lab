...
def test_report_is_generated_from_records(tmp_path: object) -> None:
    ...
    assert "140 ms" in text
    assert "Input tokens/case" in text
    assert "No prompt-transfer rows were run" in text
    assert "human-boundary metric was tested under both Mistral and Qwen" in text
    assert "`$0.00`" in text
    assert "mistral" in decision_text
    assert "Decision: **mistral**" in decision_text
