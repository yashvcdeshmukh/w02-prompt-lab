...
def render_user(
    ...
    missing = sorted(
        ...
        if name != "document_text" and name not in variables
    )
    if missing:
        raise MissingPromptVariableError(missing)

    safe_untrusted = untrusted.replace(
        DOCUMENT_MARKER_CLOSE, "&lt;/document&gt;"
    ).replace(
        CUSTOMER_MARKER_CLOSE, "&lt;/customer_message&gt;"
    )
    ...
    values["document_text"] = safe_untrusted

    def replace_placeholder(match: re.Match[str]) -> str:
        name = match.group(1)
        return values[name]

    return _PLACEHOLDER.sub(replace_placeholder, template.user_template)